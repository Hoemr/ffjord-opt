...
class ODENVP(nn.Module):
    ...
    def _logdensity(self, x, logpx=None, integration_times=None):
        _logpx = torch.zeros(x.shape[0], 1).to(x) if logpx is None else logpx
        out = []
        for idx in range(len(self.transforms)):
            x, _logpx = self.transforms[idx](x, _logpx, integration_times=integration_times)
    ...
    def _generate(self, z, logpz=None, integration_times=None):
    ...
    def forward(self, x, logpx=None, reverse=False, integration_times=None):
        if reverse:
            return self._generate(x, logpx, integration_times=integration_times)
        else:
            return self._logdensity(x, logpx, integration_times=integration_times)
