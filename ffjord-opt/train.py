...
def compute_bits_per_dim(x, model, integration_times=None):
    ...
    z, delta_logp = model(x, zero, integration_times=integration_times)  # run model forward
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data", choices=["mnist", "cifar10"], type=str, default="cifar10")
    ...
    parser.add_argument('--parallel', type=eval, default=True, choices=[True, False])
    ...
    for epoch in range(args.begin_epoch, args.num_epochs + 1):
        ...
        for _, (x, y) in enumerate(train_loader):
            ...
            # 此处加入t1的更新
            if args.time_optim:
                loss = compute_bits_per_dim(x, model, integration_times)
            else:
            ...
            if args.time_optim:
                zero = torch.zeros(x.shape[0], 1).to(x)
                z, logpz_cal = model(x, zero, integration_times=integration_times)  # run model forward
                logpz = standard_normal_logprob(z).view(z.shape[0], -1).sum(1, keepdim=True)  # logp(z)
                ...
                dlogp = dlogp.sum(2)
                dlogp = dlogp.sum(2)
                loss_diff = -(dlogp - logp_diff).mean()

                pre_t = t1.item()
                t1 = adam_t(t1.item(), loss_diff)
                time_list.append(t1.item())
                tmp = t1.item()
                del z_diff, dlog, logp_diff, t1
                ...
                with torch.no_grad():
                    ...
                    for (x, y) in test_loader:
                        ...
                        if args.time_optim:
                            ...
                            loss = compute_bits_per_dim(x, model, integration_times_eval).detach()
                        else:
                            loss = compute_bits_per_dim(x, model).detach()
                        losses.append(loss)
